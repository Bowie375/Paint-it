    ...
    with open(fn, "w") as f:
        ...
        if material is not None:
            ...
            if 'kd' in material.keys():
                if step is not None:
                    # texture.save_texture2D(os.path.join(folder, f'texture_kd_{step:04}.png'), texture.rgb_to_srgb(material['kd']))
                    texture.save_texture2D(os.path.join(folder, f'texture_kd_{step:04}.png'), material['kd'])
                else:
                    f.write('map_Kd texture_kd.png\n')
                    # texture.save_texture2D(os.path.join(folder, 'texture_kd.png'), texture.rgb_to_srgb(material['kd']))
                    texture.save_texture2D(os.path.join(folder, 'texture_kd.png'), material['kd'])

            if 'ks' in material.keys():
                if step is not None:
                    texture.save_texture2D(os.path.join(folder, f'texture_ks_{step:04}.png'), material['ks'])
                else:
            ...
            if 'normal' in material.keys():
                if step is not None:
                    texture.save_texture2D(os.path.join(folder, f'texture_n_{step:04}.png'), material['normal'], lambda_fn=lambda x:(util.safe_normalize(x)+1)*0.5)
                else:
                    f.write('map_bump texture_n.png\n')
